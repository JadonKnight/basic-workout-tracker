...
export default function TrackSets({
  ...
  // Given all of this isn't actually for state, we will likely replace with something like a useRef;
  const sets = useRef<Set[]>([]);
              ...
              <label className="flex flex-col">
                ...
                <input
                  ...
                  onChange={() => {
                    ...
                    // The issue with this is that the set cannot have undefined values but this passes undefined values
                    // when the previous set doesn't exist

                    // I need to redefine the schema to allow null values (except reps which always exist)
                    // and then redefine the Set parameter
                    // setSets((prev) => {
                    //   const updatedSet = {
                    //     weight: Number(e.target.value),
                    //     ...(prev[index] || {}), // Provide a default empty object if prev[index] is undefined
                    //   };
                    //   return [
                    //     ...prev.slice(0, index), // Keep the sets before the updated set
                    //     updatedSet, // Update the weight property
                    //     ...prev.slice(index + 1), // Keep the sets after the updated set
                    //   ];
                    // });
                  }}
                />
            ...
            <div className="flex flex-col">
              <label className="flex flex-col">
                ...
                <input
                  ...
                  value={set.reps || ""}
                  onChange={(e) => {
                    const newPrevSets = JSON.parse(JSON.stringify(prevSessionSets));
